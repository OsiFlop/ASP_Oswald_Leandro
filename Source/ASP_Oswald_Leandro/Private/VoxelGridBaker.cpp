...
void AVoxelGridBaker::DebugDrawSomeCells(int32 MaxCellsToDraw)
{
	...
	for (int32 i = 0; i < Count; ++i)
	{
		...
		DrawDebugLine(GetWorld(), A, B, FColor::Green, false, GridConfig->DebugDrawLifetime, 0, 1.0f);
		DrawDebugLine(GetWorld(), B, C, FColor::Green, false, GridConfig->DebugDrawLifetime, 0, 1.0f);
		DrawDebugLine(GetWorld(), C, D, FColor::Green, false, GridConfig->DebugDrawLifetime, 0, 1.0f);
		DrawDebugLine(GetWorld(), D, A, FColor::Green, false, GridConfig->DebugDrawLifetime, 0, 1.0f);
	}
}

void AVoxelGridBaker::DebugDrawSomeCells50()
{
	DebugDrawSomeCells(5000);
}
